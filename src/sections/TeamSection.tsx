...
const TeamSection: React.FC<TeamSectionProps> = ({ limit = 7 }) => {
  ...
  const fetchTeamMembers = async () => {
    try {
      ...
      const sortedMembers = response.teamMembers.sort(
  ...
  // Function to render a team member card
  const renderTeamCard = (member: Team, idx: number, isLarge = false) => (
    <div
      key={member.id}
      className={`bg-white dark:bg-gray-900/95 backdrop-blur-xl shadow-2xl overflow-hidden border border-gray-200 dark:border-gray-600/30 rounded-lg group hover:border-gray-300 dark:hover:border-gray-500/50 transition-all duration-300 ${
    ...
    <section className="py-16 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {teamMembers.map((member, idx) => renderTeamCard(member, idx))}
        </div>
      </div>
    </section>
  );
